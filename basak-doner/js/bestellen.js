/* Bossche Başak Döner: het online bestelsysteem, powered by Mettafel.
   Alleen afhalen (de zaak bezorgt bewust niet), zoeken en categoriechips,
   en een licht "account op dit toestel" (localStorage, geen wachtwoord)
   met bestel-opnieuw. Fail-open: zonder JavaScript blijft de volledige
   kaart leesbaar; de plusknoppen doen dan niets en de contactpagina vangt
   vragen op. Demo-afspraak: geen online betaling, betalen bij afhalen.
   Openingstijden: elke dag 10.00-20.00, vrijdag tot 21.00 (bron: huidige
   site; eigenaar bevestigt, zie README). Laatste afhaalmoment een kwartier
   voor sluiting, kwartierstappen. */
(function () {
  'use strict';

  var T = {
    leeg: 'Nog leeg. Kies iets lekkers van de kaart hiernaast.',
    naamFout: 'Vul je naam in, dan weten we voor wie we gaan snijden.',
    telFout: 'Vul een telefoonnummer in waarop we je kunnen bereiken.',
    mandLeegFout: 'Je mandje is nog leeg.',
    verstuurFout: function (m) { return 'Versturen lukte niet (' + m + '). Probeer het opnieuw, of loop gewoon even binnen.'; },
    versturen: 'Versturen...', plaats: 'Plaats bestelling',
    zsm: 'Zo snel mogelijk (± 15 minuten)',
    vandaag: 'Vandaag',
    bevestig: function (naam, n, tot, tijd) { return 'Dankjewel, ' + naam + '! We snijden vers voor je van het spit. Je bestelling (' + n + ' items, ' + tot + ') staat klaar: ' + tijd + ', aan de Helftheuvelpassage 54. Betalen doe je bij het afhalen, met pin of contant.'; },
    ...
    accountHoi: function (naam) { return 'Hoi ' + naam + ', welkom terug!'; },
    accountWissen: 'gegevens wissen',
    opnieuw: function (n, tot) { return 'Bestel opnieuw: ' + n + ' items · ' + tot; },
  ...
  var mand = {};
  var bewaard = leesBlok('basak-mand') || {};
  ...
  function bewaar() { bewaarBlok('basak-mand', mand); }
  ...
  if (zwever) {
    zwever.addEventListener('click', function () {
      document.getElementById('mandje').scrollIntoView({ block: 'start', behavior: 'smooth' });
    });
    zwever.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); zwever.click(); }
    });
  }
  ...
  /* ---------- afhaaltijden binnen de openingstijden ----------
     Elke dag 10.00-20.00, vrijdag tot 21.00. Laatste moment een kwartier
     voor sluiting, kwartierstappen, voorbereiding 15 minuten. */
  function sluitUur(dag) { return dag === 5 ? 21 : 20; }
  function tijden() {
    var nu = new Date();
    var lijst = [];
    var dag = new Date(nu);
    var voorbereiding = 15;
    for (var d = 0; d < 8 && lijst.length === 0; d++) {
      var van = new Date(dag); van.setHours(10, 0, 0, 0);
      var tot = new Date(dag); tot.setHours(sluitUur(dag.getDay()) - 1, 45, 0, 0);
  ...
  function profiel() { return leesBlok('basak-profiel'); }
  function laatste() { return leesBlok('basak-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('basak-profiel'); localStorage.removeItem('basak-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('basak-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
    ...
    try {
      var r = await fetch('/api/bestelling', {
        ...
        body: JSON.stringify({
          ...
          regels: regels,
          totaal: Math.round(subtotaal() * 100) / 100,
        }),
      });
      var j = await r.json();
      ...
      var n = aantalStuks(), tot = euro(subtotaal());
      document.querySelector('[data-bevestig-nummer]').textContent = 'Bestelling ' + j.id;
      document.querySelector('[data-bevestig-tekst]').textContent = T.bevestig(naam, n, tot, tijdTekst);

      bewaarBlok('basak-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('basak-profiel', { naam: naam, telefoon: telefoon });
      } else {
