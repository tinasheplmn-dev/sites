/* Eethuis GULLI: het online bestelsysteem, powered by Mettafel.
   Afhalen of bezorgen, zoeken en categoriechips, en een licht
   "account op dit toestel" (localStorage, geen wachtwoord) met bestel-opnieuw.
   Fail-open: zonder JavaScript blijft de volledige kaart leesbaar;
   de plusknoppen doen dan niets en de contactroute blijft staan.
...
(function () {
  ...
  var T = {
    ...
    verstuurFout: function (m) { return 'Versturen lukte niet (' + m + '). Probeer het zo opnieuw, of bel ons even op 06 84288414.'; },
    ...
    bevestigBezorg: function (naam, n, tot, tijd, adres) { return 'Bedankt ' + naam + '! We gaan voor je aan de slag. Je bestelling (' + n + ' items, ' + tot + ') komt eraan: ' + tijd + ', op ' + adres + '. Betalen doe je aan de deur, met pin of contant.'; },
    bevestigAfhaal: function (naam, n, tot, tijd) { return 'Bedankt ' + naam + '! We gaan voor je aan de slag. Je bestelling (' + n + ' items, ' + tot + ') staat klaar: ' + tijd + ', aan de Christiaan Huygensweg 137. Betalen doe je bij het afhalen, met pin of contant.'; },
  ...
  var mand = {};
  var bewaard = leesBlok('gulli-mand') || {};
  ...
  function bewaar() { bewaarBlok('gulli-mand', mand); }
  ...
  /* ---------- tijden binnen de echte openingstijden ----------
     ma t/m za 11.00-19.30, zondag dicht; kwartierstappen, laatste slot 19.15 */
  var wijze = 'afhalen';
  ...
  function tijden() {
    ...
    for (var d = 0; d < 8 && lijst.length === 0; d++) {
      if (open(dag.getDay())) {
        var van = new Date(dag); van.setHours(11, 0, 0, 0);
        var tot = new Date(dag); tot.setHours(19, 15, 0, 0);
        var lead = wijze === 'bezorgen' ? 45 : 20;
        var vroegst = new Date(Math.max(van.getTime(), nu.getTime() + lead * 60000));
  ...
  function profiel() { return leesBlok('gulli-profiel'); }
  function laatste() { return leesBlok('gulli-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('gulli-profiel'); localStorage.removeItem('gulli-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('gulli-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
  ...
  stapAfronden.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      bewaarBlok('gulli-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('gulli-profiel', { naam: naam, telefoon: telefoon, adres: adres, postcode: postcode });
      } else {
