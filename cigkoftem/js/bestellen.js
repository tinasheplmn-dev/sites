/* Çiğköftem Den Bosch: het online bestelsysteem, powered by Mettafel.
   Afhalen of bezorgen, zoeken en categoriechips, en een licht "account op
   dit toestel" (localStorage, geen wachtwoord) met bestel-opnieuw.
   Fail-open: zonder JavaScript blijft de volledige kaart leesbaar; de
   plusknoppen doen dan niets en de contactpagina vangt vragen op.
   Demo-afspraak: geen online betaling, betalen bij ontvangst.
   Bezorgkosten/-gebied zijn nog niet bevestigd door de eigenaar: er wordt
   bewust niets opgeteld of beloofd (zie README, open punten). */
(function () {
  'use strict';
  ...
  var T = {
    ...
    zsmAfhaal: 'Zo snel mogelijk (± 15 minuten)', zsmBezorg: 'Zo snel mogelijk',
    vandaag: 'Vandaag', tijdAfhaal: 'Afhaaltijd', tijdBezorg: 'Bezorgtijd',
    bevestigBezorg: function (naam, n, tot, tijd, adres) { return 'Teşekkürler, ' + naam + '! We gaan vers voor je draaien. Je bestelling (' + n + ' items, ' + tot + ') komt eraan: ' + tijd + ', op ' + adres + '. Betalen doe je aan de deur, met pin of contant.'; },
    bevestigAfhaal: function (naam, n, tot, tijd) { return 'Teşekkürler, ' + naam + '! We gaan vers voor je draaien. Je bestelling (' + n + ' items, ' + tot + ') staat klaar: ' + tijd + ', Christiaan Huygensweg 133 (bij Sonay Supermarkt). Betalen doe je bij het afhalen, met pin of contant.'; },
    ...
    accountHoi: function (naam) { return 'Merhaba, ' + naam + '!'; },
  ...
  var mand = {};
  var bewaard = leesBlok('cigkoftem-mand') || {};
  ...
  /* ---------- tijden binnen de openingstijden ----------
     ma t/m za 12.00-22.00, zondag gesloten (werkhypothese uit het Google-
     profiel; eigenaar bevestigt, zie README). Laatste moment 21.45,
     kwartierstappen. */
  function open(dag) { return dag !== 0; }
  function tijden() {
    var nu = new Date();
    var lijst = [];
    var dag = new Date(nu);
    var voorbereiding = wijze === 'bezorgen' ? 30 : 15;
  ...
  function profiel() { return leesBlok('cigkoftem-profiel'); }
  function laatste() { return leesBlok('cigkoftem-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('cigkoftem-profiel'); localStorage.removeItem('cigkoftem-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('cigkoftem-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
  ...
  stapAfronden.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      bewaarBlok('cigkoftem-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('cigkoftem-profiel', { naam: naam, telefoon: telefoon, adres: adres, postcode: postcode });
      } else {
