/* Friethuys Kwibus: het online bestelsysteem, powered by Mettafel.
   Vooruit bestellen om mee te nemen of om in de zaak op te eten
   (binnenlopen en aan de balie bestellen kan altijd), zoeken en
   categoriechips, een licht "account op dit toestel" (localStorage,
   ...
   Demo-afspraak: geen online betaling, betalen bij het afhalen. */
(function () {
  'use strict';

  var EN = document.documentElement.lang === 'en';
  var T = EN ? {
    leeg: 'Still empty. Pick something tasty from the menu.',
    ...
    verstuurFout: function (m) { return 'Sending failed (' + m + '). Please try again, or simply order at the counter.'; },
    versturen: 'Sending...', plaats: 'Place order',
    zsm: 'As soon as possible',
    vandaag: 'Today', tijdAfhaal: 'Collection time',
    bevestig: function (naam, n, tot, tijd, hier) { return 'Thanks ' + naam + '! We’re on it. Your order (' + n + ' items, ' + tot + ') will be ready ' + (hier ? 'to eat in' : 'to take away') + ': ' + tijd + ', at Onderwijsboulevard 416. You pay at the counter, by card or in cash.'; },
    ...
    verstuurFout: function (m) { return 'Versturen lukte niet (' + m + '). Probeer het opnieuw, of bestel gewoon aan de balie.'; },
    versturen: 'Versturen...', plaats: 'Plaats bestelling',
    zsm: 'Zo snel mogelijk',
    vandaag: 'Vandaag', tijdAfhaal: 'Afhaaltijd',
    bevestig: function (naam, n, tot, tijd, hier) { return 'Bedankt ' + naam + '! We gaan voor je aan de slag. Je bestelling (' + n + ' producten, ' + tot + ') staat ' + (hier ? 'klaar om hier op te eten' : 'klaar om mee te nemen') + ': ' + tijd + ', Onderwijsboulevard 416. Betalen doe je aan de balie, met pin of contant.'; },
    ...
    opnieuw: function (n, tot) { return 'Bestel opnieuw: ' + n + ' producten · ' + tot; },
  ...
  var mand = {};
  var bewaard = leesBlok('kwibus-mand') || {};
  ...
  function bewaar() { bewaarBlok('kwibus-mand', mand); }
  ...
  if (zoek) {
    zoek.addEventListener('input', function () {
      ...
      cats.forEach(function (cat) {
        ...
        cat.querySelectorAll('.menu-groep-kop, .menu-groep-noot').forEach(function (k) { k.hidden = !!q; });
        cat.hidden = !!q && !inCat;
  ...
  /* ---------- afhaaltijden binnen de echte openingstijden ----------
     Elke dag open: ma-vr 11.30-20.30, za-zo 16.30-20.30 (bron: het
     openingstijden-endpoint van de zaak zelf, 13-09-2026). Slots per
     kwartier, laatste slot 20.30; de keuken heeft even bereidingstijd. */
  function vanUur(dag) { return (dag >= 1 && dag <= 5) ? 11.5 : 16.5; }
  function tijden() {
    var nu = new Date();
    var lijst = [];
    var dag = new Date(nu);
    for (var d = 0; d < 8 && lijst.length === 0; d++) {
      var vu = vanUur(dag.getDay());
      var van = new Date(dag); van.setHours(Math.floor(vu), (vu % 1) * 60, 0, 0);
      var tot = new Date(dag); tot.setHours(20, 30, 0, 0);
      var vroegst = new Date(Math.max(van.getTime(), nu.getTime() + 15 * 60000));
      vroegst.setMinutes(Math.ceil(vroegst.getMinutes() / 15) * 15, 0, 0);
      if (vroegst <= tot) {
        var vandaag = dag.toDateString() === nu.toDateString();
        if (vandaag && nu >= van) {
          lijst.push({ w: 'zsm', t: T.zsm });
        }
  ...
  var eetwijze = 'meenemen';
  var eetWrap = document.getElementById('eetwijze');
  if (eetWrap) {
    eetWrap.addEventListener('click', function (e) {
      var b = e.target.closest('[data-eetwijze]');
      if (!b) return;
      eetwijze = b.dataset.eetwijze;
      eetWrap.querySelectorAll('.slot').forEach(function (k) { k.setAttribute('aria-pressed', String(k === b)); });
    });
  }
  function vulTijden() {
  ...
  function profiel() { return leesBlok('kwibus-profiel'); }
  function laatste() { return leesBlok('kwibus-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('kwibus-profiel'); localStorage.removeItem('kwibus-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('kwibus-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
    ...
    try {
      var r = await fetch('/api/bestelling', {
        ...
        body: JSON.stringify({
          naam: naam,
          telefoon: telefoon,
          wijze: eetwijze === 'hier-eten' ? 'hier opeten' : 'meenemen',
          tijd: select.value,
      ...
      document.querySelector('[data-bevestig-tekst]').textContent = T.bevestig(naam, n, tot, tijdTekst, eetwijze === 'hier-eten');

      /* laatste bestelling bewaren; profiel bijwerken of aanbieden */
      bewaarBlok('kwibus-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('kwibus-profiel', { naam: naam, telefoon: telefoon });
      } else {
