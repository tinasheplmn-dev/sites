/* Al Mundo: het online bestelsysteem, powered by Mettafel.
   Alleen afhalen (Al Mundo bezorgt niet), zoeken en categoriechips, favorietenlabels,
...
(function () {
  ...
  var T = EN ? {
    ...
    verstuurFout: function (m) { return 'Sending failed (' + m + '). Please try again, or call us on 073 610 0564.'; },
    ...
    bevestigAfhaal: function (naam, n, tot, tijd) { return 'Thank you ' + naam + "! We're on it. Your order (" + n + ' dishes, ' + tot + ') will be ready: ' + tijd + ', at Orthenstraat 284. You pay on collection, by card or in cash.'; },
    ...
    verstuurFout: function (m) { return 'Versturen lukte niet (' + m + '). Probeer het opnieuw, of bel ons op 073 610 0564.'; },
    ...
    bevestigAfhaal: function (naam, n, tot, tijd) { return 'Dank je wel ' + naam + '! We gaan voor je aan de slag. Je bestelling (' + n + ' gerechten, ' + tot + ') staat klaar: ' + tijd + ', Orthenstraat 284. Betalen doe je bij het afhalen, met pin of contant.'; },
  ...
  var mand = {};
  var bewaard = leesBlok('almundo-mand') || {};
  ...
  function bewaar() { bewaarBlok('almundo-mand', mand); }
  ...
  /* di t/m zo; telefonisch en online afhalen kan van 16:00 tot 20:00
     (bron: huidige site); laatste moment 19:45, kwartierstappen */
  var wijze = 'afhalen';
  ...
  function profiel() { return leesBlok('almundo-profiel'); }
  function laatste() { return leesBlok('almundo-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('almundo-profiel'); localStorage.removeItem('almundo-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('almundo-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
  ...
  stapAfronden.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      bewaarBlok('almundo-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('almundo-profiel', { naam: naam, telefoon: telefoon, adres: adres, postcode: postcode });
      } else {
