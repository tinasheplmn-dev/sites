/* Stefano's: reserveringswidget, powered by Mettafel. Tweetalig (nl/en).
   Reserveren is hier alleen nodig VANAF 5 PERSONEN; met vier of minder loop je
   gewoon binnen. Dat staat bovenaan het paneel en de chips beginnen bij 5.
...
(function () {
  ...
  var T = EN ? {
    ...
    dicht: "We're open every day from 4.30 pm. Pick a day.",
    ...
    groep: 'For groups of 13 or more we confirm personally: send us a message via the <a href="' + 'CONTACTLINK' + '" style="font-weight:700">contact form</a>.',
    groepFout: 'For groups of 13 or more: send us a message via the contact form and we\'ll confirm personally.',
    ...
    bevestigd: function (naam, p, datum, tijd) { return 'Grazie ' + naam + '! Your table for ' + p + (p === 1 ? ' person' : ' people') + ' is confirmed for ' + datum + ' at ' + tijd + '. See you at Stoofstraat 10!'; },
    reservering: 'Reservation ', agenda: 'Add to your calendar',
    binnenlopen: 'Up to 4 people you can simply walk in. From 5 we set a table for you.',
    titel: 'Book a table', zwever: 'Book', sluiten: 'Close', powered: 'Reservation system powered by',
    icsTitel: "Dinner at Stefano's",
  } : {
    ...
    dicht: 'We zijn elke dag open vanaf 16.30 uur. Kies een dag.',
    ...
    groep: 'Voor groepen vanaf 13 personen bevestigen we persoonlijk: stuur ons een bericht via het <a href="' + 'CONTACTLINK' + '" style="font-weight:700">contactformulier</a>.',
    groepFout: 'Voor groepen vanaf 13 personen: stuur ons een bericht via het contactformulier, dan bevestigen we persoonlijk.',
    ...
    bevestigd: function (naam, p, datum, tijd) { return 'Grazie ' + naam + '! Je tafel voor ' + p + (p === 1 ? ' persoon' : ' personen') + ' staat vast op ' + datum + ' om ' + tijd + ' uur. Tot dan, op Stoofstraat 10!'; },
    reservering: 'Reservering ', agenda: 'Zet in je agenda',
    binnenlopen: 'Tot en met 4 personen loop je gewoon binnen. Vanaf 5 zetten we een tafel voor je klaar.',
    titel: 'Reserveer een tafel', zwever: 'Reserveer', sluiten: 'Sluiten', powered: 'Reserveringssysteem powered by',
    icsTitel: "Eten bij Stefano's",
  };
  T.groep = T.groep.replace('CONTACTLINK', CONTACT);

  var VANAF = 5;                 /* reserveren vanaf dit aantal personen */
  var GROEP = 13;                /* hierboven bevestigen we persoonlijk */
  var CAPACITEIT = 20;
  var SLOTS = ['17:00', '17:30', '18:00', '18:30', '19:00', '19:30', '20:00', '20:30'];
  /* Stefano's is elke dag open vanaf 16.30; zo t/m wo tot 21.00, do t/m za tot 21.30.
     Het laatste tijdslot houdt daar een uur eten op over. */
  function openDag() { return true; }
  function laatsteSlot(d) { return (d.getDay() >= 4 && d.getDay() <= 6) ? '20:30' : '20:00'; }
  function slotsVoor(d) { var laat = laatsteSlot(d); return SLOTS.filter(function (t) { return t <= laat; }); }
  ...
  function dagOpties(aantal) {
    ...
    for (var i = 0; i < 21 && uit.length < aantal; i++) {
      if (openDag(d)) {
        var laat = laatsteSlot(d).split(':');
        var laatste = new Date(d); laatste.setHours(parseInt(laat[0], 10), parseInt(laat[1], 10), 0, 0);
  ...
  var MARKUP = '' +
    ...
    '    <p class="rw-binnenlopen">' + T.binnenlopen + '</p>' +
    '    <div class="veld">' +
    ...
    [5, 6, 7, 8, 9, 10, 11, 12].map(function (n) {
      return '<button type="button" class="slot" data-p="' + n + '" aria-pressed="' + (n === 5) + '">' + n + '</button>';
    }).join('') +
    '        <button type="button" class="slot" data-p="13" aria-pressed="false">13+</button>' +
    '      </div>' +
    ...
    '    <p data-rw-tekst></p>' +
    '    <a class="knop knop-lijn" data-rw-agenda hidden download="stefanos.ics"><svg class="ikoon" aria-hidden="true"><use href="assets/iconen.svg#i-kalender"/></svg> ' + T.agenda + '</a>' +
    '  </div>' +
    '</div>';

  function Widget(wortel) {
    var q = function (sel) { return wortel.querySelector(sel); };
    var personen = VANAF, datum = null, tijd = null, bezet = {}, capaciteit = CAPACITEIT;
    ...
    function toonTijden() {
      ...
      tijdenWrap.innerHTML = slotsVoor(new Date(datum + 'T12:00:00')).map(function (t) {
        if (isVandaag) {
          var um = t.split(':');
    ...
    q('[data-rw-personen]').addEventListener('click', function (e) {
      ...
      groepNoot.hidden = personen < GROEP;
      if (datum) toonTijden();
    });
    ...
    q('[data-rw-verstuur]').addEventListener('click', async function () {
      ...
      var ok = true;
      if (personen >= GROEP) { fout('algemeen', T.groepFout); ok = false; }
      ...
      try {
        ...
        if (agenda && j.status !== 'wachtlijst') {
          ...
          var ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Mettafel//Stefanos Pizzeria//NL', 'BEGIN:VEVENT',
            'UID:' + j.id + '@stefano-s.nl',
            'DTSTART:' + stamp, 'DTEND:' + eindStamp,
            'SUMMARY:' + T.icsTitel + ' (' + j.id + ')',
            'LOCATION:Stoofstraat 10\\, 5211 ER \'s-Hertogenbosch',
  ...
  if (!inline) {
    ...
    overlay.innerHTML =
      ...
      '    <div><span class="eyebrow">Stefano\u2019s</span><h2>' + T.titel + '</h2></div>' +
    ...
    if (!document.querySelector('[data-mandje-zwever]')) {
      ...
      zwever.innerHTML = '<svg class="ikoon" aria-hidden="true"><use href="assets/iconen.svg#i-kalender"/></svg> ' + T.zwever;
