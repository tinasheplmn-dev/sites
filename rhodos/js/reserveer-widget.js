/* Restaurant Rhodos: reserveringswidget, powered by Mettafel. Tweetalig (nl/en).
...
(function () {
  ...
  var T = EN ? {
    ...
    dicht: "We're open every day. Pick a day above.",
    ...
    groep: 'For groups of 9 or more (up to 70, arrangements) we confirm personally: send us a message via the <a href="' + 'CONTACTLINK' + '" style="font-weight:700">contact form</a>.',
    ...
    bevestigd: function (naam, p, datum, tijd) { return 'Efcharisto ' + naam + '! Your table for ' + p + (p === 1 ? ' person' : ' people') + ' is confirmed for ' + datum + ' at ' + tijd + '. See you at Kerkstraat 87!'; },
    ...
    icsTitel: 'Dinner at Restaurant Rhodos',
  } : {
    ...
    dicht: 'We zijn elke dag open. Kies hierboven een dag.',
    ...
    groep: 'Voor groepen vanaf 9 personen (tot 70, arrangementen) bevestigen we persoonlijk: stuur ons een bericht via het <a href="' + 'CONTACTLINK' + '" style="font-weight:700">contactformulier</a>.',
    ...
    bevestigd: function (naam, p, datum, tijd) { return 'Efcharisto ' + naam + '! Je tafel voor ' + p + (p === 1 ? ' persoon' : ' personen') + ' staat vast op ' + datum + ' om ' + tijd + ' uur. Tot dan, aan de Kerkstraat 87!'; },
    ...
    icsTitel: 'Diner bij Restaurant Rhodos',
  };
  T.groep = T.groep.replace('CONTACTLINK', CONTACT);

  /* Restaurant Rhodos: capaciteit per tijdslot van een half uur. Gasten
     worden over drie verdiepingen verdeeld; 28 is een conservatieve
     inschatting die de eigenaar hier kan bijstellen. */
  var CAPACITEIT = 28;
  /* open ma t/m zo, geen sluitingsdag. Tijdsloten per half uur van 15:30
     t/m 21:00; op zaterdag is er ook lunch, dus dan lopen de sloten door
     vanaf 11:30 t/m 21:00. */
  function openDag(d) { return true; }
  function slotsVoorDag(d) {
    var vanUur = d.getDay() === 6 ? [11, 30] : [15, 30];
    var lijst = [];
    var t = new Date(d); t.setHours(vanUur[0], vanUur[1], 0, 0);
    var eind = new Date(d); eind.setHours(21, 0, 0, 0);
    while (t <= eind) {
      lijst.push(String(t.getHours()).padStart(2, '0') + ':' + String(t.getMinutes()).padStart(2, '0'));
  ...
  var MARKUP = '' +
    ...
    '    <p data-rw-tekst></p>' +
    '    <a class="knop knop-lijn" data-rw-agenda hidden download="restaurant-rhodos.ics"><svg class="ikoon" aria-hidden="true"><use href="assets/iconen.svg#i-kalender"/></svg> ' + T.agenda + '</a>' +
    '  </div>' +
    '</div>';
  ...
  function Widget(wortel) {
    ...
    function toonTijden() {
      ...
      var slots = slotsVoorDag(new Date(datum + 'T12:00'));
    ...
    q('[data-rw-verstuur]').addEventListener('click', async function () {
      ...
      try {
        ...
        if (agenda && j.status !== 'wachtlijst') {
          ...
          var ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Mettafel//Restaurant Rhodos//NL', 'BEGIN:VEVENT',
            'UID:' + j.id + '@restaurant-rhodos.com',
            'DTSTART:' + stamp, 'DTEND:' + eindStamp,
            'SUMMARY:' + T.icsTitel + ' (' + j.id + ')',
            'LOCATION:Kerkstraat 87\\, 5211 KE \'s-Hertogenbosch',
  ...
  if (!inline) {
    ...
    overlay.innerHTML =
      ...
      '    <div><span class="eyebrow">Restaurant Rhodos</span><h2>' + T.titel + '</h2></div>' +
