/* Irakees Restaurant Bagdad: reserveringswidget, powered by Mettafel. Tweetalig (nl/en).
...
(function () {
  ...
  var T = EN ? {
    ...
    bevestigd: function (naam, p, datum, tijd) { return 'Thank you ' + naam + '! Your table for ' + p + (p === 1 ? ' person' : ' people') + ' is confirmed for ' + datum + ' at ' + tijd + '. See you at Orthenstraat 36!'; },
    ...
    icsTitel: 'Dinner at Restaurant Bagdad',
  } : {
    ...
    bevestigd: function (naam, p, datum, tijd) { return 'Dank je wel ' + naam + '! Je tafel voor ' + p + (p === 1 ? ' persoon' : ' personen') + ' staat vast op ' + datum + ' om ' + tijd + ' uur. Tot dan, aan de Orthenstraat 36!'; },
    ...
    icsTitel: 'Diner bij Restaurant Bagdad',
  };
    ...
    q('[data-rw-verstuur]').addEventListener('click', async function () {
      ...
      try {
        ...
        if (agenda && j.status !== 'wachtlijst') {
          ...
          var ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Mettafel//Bagdad//NL', 'BEGIN:VEVENT',
            'UID:' + j.id + '@al-mundo.nl',
            'DTSTART:' + stamp, 'DTEND:' + eindStamp,
            'SUMMARY:' + T.icsTitel + ' (' + j.id + ')',
            'LOCATION:Orthenstraat 36\\, 5211 SX \'s-Hertogenbosch',
  ...
  if (!inline) {
    ...
    overlay.innerHTML =
      ...
      '    <div><span class="eyebrow">Restaurant Bagdad</span><h2>' + T.titel + '</h2></div>' +
