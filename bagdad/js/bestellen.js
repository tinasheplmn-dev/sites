/* Irakees Restaurant Bagdad: het online bestelsysteem, powered by Mettafel.
   Alleen afhalen (Bagdad bezorgt niet), zoeken en categoriechips, favorietenlabels,
...
(function () {
  ...
  var T = EN ? {
    ...
    verstuurFout: function (m) { return 'Sending failed (' + m + '). Please try again, or call us on 073 785 9211.'; },
    versturen: 'Sending...', plaats: 'Place order',
    zsmAfhaal: 'As soon as possible (about 25 minutes)', zsmBezorg: 'As soon as possible (± 45 minutes)',
    vandaag: 'Today', tijdAfhaal: 'Collection time', tijdBezorg: 'Delivery time',
    bevestigBezorg: function (naam, n, tot, tijd, adres) { return 'Grazie ' + naam + "! We're on it. Your order (" + n + ' dishes, ' + tot + ') is on its way: ' + tijd + ', to ' + adres + '. You pay at the door, by card or in cash.'; },
    bevestigAfhaal: function (naam, n, tot, tijd) { return 'Thank you ' + naam + "! We're on it. Your order (" + n + ' dishes, ' + tot + ') will be ready: ' + tijd + ', at Orthenstraat 36. You pay on collection, by card or in cash.'; },
    ...
    verstuurFout: function (m) { return 'Versturen lukte niet (' + m + '). Probeer het opnieuw, of bel ons op 073 785 9211.'; },
    versturen: 'Versturen...', plaats: 'Plaats bestelling',
    zsmAfhaal: 'Zo snel mogelijk (ongeveer 25 minuten)', zsmBezorg: 'Zo snel mogelijk (± 45 minuten)',
    ...
    bevestigAfhaal: function (naam, n, tot, tijd) { return 'Dank je wel ' + naam + '! We gaan voor je aan de slag. Je bestelling (' + n + ' gerechten, ' + tot + ') staat klaar: ' + tijd + ', Orthenstraat 36. Betalen doe je bij het afhalen, met pin of contant.'; },
  ...
  var mand = {};
  var bewaard = leesBlok('bagdad-mand') || {};
  ...
  function bewaar() { bewaarBlok('bagdad-mand', mand); }
  ...
  /* di t/m zo; het restaurant is open van 17:00 tot 22:00 (bron: huidige
     site); afhalen kan vanaf 17:00, laatste moment 21:30, kwartierstappen */
  var wijze = 'afhalen';
  ...
  function tijden() {
    ...
    for (var d = 0; d < 8 && lijst.length === 0; d++) {
      if (open(dag.getDay())) {
        var van = new Date(dag); van.setHours(17, 0, 0, 0);
        var tot = new Date(dag); tot.setHours(21, 30, 0, 0);
  ...
  function profiel() { return leesBlok('bagdad-profiel'); }
  function laatste() { return leesBlok('bagdad-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('bagdad-profiel'); localStorage.removeItem('bagdad-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('bagdad-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
  ...
  stapAfronden.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      bewaarBlok('bagdad-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('bagdad-profiel', { naam: naam, telefoon: telefoon, adres: adres, postcode: postcode });
      } else {
