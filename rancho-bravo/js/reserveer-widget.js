/* Rancho Bravo: reserveringswidget, powered by Mettafel. Tweetalig (nl/en).
...
(function () {
  ...
  var T = EN ? {
    ...
    dicht: 'Pick a date within the next two months.',
    ...
    bevestigd: function (naam, p, datum, tijd) { return 'Gracias ' + naam + '! Your table for ' + p + (p === 1 ? ' person' : ' people') + ' is confirmed for ' + datum + ' at ' + tijd + '. See you at Orthenstraat 53!'; },
    ...
    icsTitel: 'Dinner at Rancho Bravo',
  } : {
    ...
    dicht: 'Kies een datum binnen de komende twee maanden.',
    ...
    bevestigd: function (naam, p, datum, tijd) { return '\u00a1Gracias ' + naam + '! Je tafel voor ' + p + (p === 1 ? ' persoon' : ' personen') + ' staat vast op ' + datum + ' om ' + tijd + ' uur. Tot dan, aan de Orthenstraat 53!'; },
    ...
    icsTitel: 'Diner bij Rancho Bravo',
  };
  T.groep = T.groep.replace('CONTACTLINK', CONTACT);

  var CAPACITEIT = 20;
  /* elke dag open: ma t/m do 16:30-22:00, vr t/m zo 16:00-22:30 */
  function weekendDag(dag) { return dag === 0 || dag === 5 || dag === 6; }
  function slotsVoor(datumIso) {
    var d = new Date(datumIso + 'T12:00');
    return weekendDag(d.getDay())
  ...
  function dagOpties(aantal) {
    ...
    for (var i = 0; i < 21 && uit.length < aantal; i++) {
      if (openDag(d)) {
        var laatste = new Date(d); laatste.setHours(weekendDag(d.getDay()) ? 20 : 20, weekendDag(d.getDay()) ? 45 : 15, 0, 0);
  ...
  var MARKUP = '' +
    ...
    '    <p data-rw-tekst></p>' +
    '    <a class="knop knop-lijn" data-rw-agenda hidden download="rancho-bravo.ics"><svg class="ikoon" aria-hidden="true"><use href="assets/iconen.svg#i-kalender"/></svg> ' + T.agenda + '</a>' +
    '  </div>' +
    '</div>';
  ...
  function Widget(wortel) {
    ...
    function toonTijden() {
      ...
      tijdenWrap.innerHTML = slotsVoor(datum).map(function (t) {
        if (isVandaag) {
          var um = t.split(':');
    ...
    q('[data-rw-verstuur]').addEventListener('click', async function () {
      ...
      try {
        ...
        if (agenda && j.status !== 'wachtlijst') {
          ...
          var ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Mettafel//Rancho Bravo//NL', 'BEGIN:VEVENT',
            'UID:' + j.id + '@ranchobravo.nl',
            'DTSTART:' + stamp, 'DTEND:' + eindStamp,
            'SUMMARY:' + T.icsTitel + ' (' + j.id + ')',
            'LOCATION:Orthenstraat 53\\, 5211 SV \'s-Hertogenbosch',
  ...
  if (!inline) {
    ...
    overlay.innerHTML =
      ...
      '    <div><span class="eyebrow">Rancho Bravo</span><h2>' + T.titel + '</h2></div>' +
