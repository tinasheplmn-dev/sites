/* Rancho Bravo: het online bestelsysteem, powered by Mettafel.
...
(function () {
  ...
  var T = EN ? {
    ...
    verstuurFout: function (m) { return 'Sending failed (' + m + '). Please try again, or call us on 073 613 0167.'; },
    versturen: 'Sending...', plaats: 'Place order',
    zsmAfhaal: 'As soon as possible (± 25 minutes)', zsmBezorg: 'As soon as possible (± 45 minutes)',
    vandaag: 'Today', tijdAfhaal: 'Collection time', tijdBezorg: 'Delivery time',
    bevestigBezorg: function (naam, n, tot, tijd, adres) { return 'Gracias ' + naam + "! We're on it. Your order (" + n + ' dishes, ' + tot + ') is on its way: ' + tijd + ', to ' + adres + '. You pay at the door, by card or in cash.'; },
    bevestigAfhaal: function (naam, n, tot, tijd) { return 'Gracias ' + naam + "! We're on it. Your order (" + n + ' dishes, ' + tot + ') will be ready: ' + tijd + ', at Orthenstraat 53. You pay on collection, by card or in cash.'; },
    ...
    accountHoi: function (naam) { return '\u00a1Hola ' + naam + '!'; },
    ...
    verstuurFout: function (m) { return 'Versturen lukte niet (' + m + '). Probeer het opnieuw, of bel ons op 073 613 0167.'; },
    versturen: 'Versturen...', plaats: 'Plaats bestelling',
    zsmAfhaal: 'Zo snel mogelijk (± 25 minuten)', zsmBezorg: 'Zo snel mogelijk (± 45 minuten)',
    vandaag: 'Vandaag', tijdAfhaal: 'Afhaaltijd', tijdBezorg: 'Bezorgtijd',
    bevestigBezorg: function (naam, n, tot, tijd, adres) { return '\u00a1Gracias ' + naam + '! We gaan voor je aan de slag. Je bestelling (' + n + ' gerechten, ' + tot + ') komt eraan: ' + tijd + ', op ' + adres + '. Betalen doe je aan de deur, met pin of contant.'; },
    bevestigAfhaal: function (naam, n, tot, tijd) { return '\u00a1Gracias ' + naam + '! We gaan voor je aan de slag. Je bestelling (' + n + ' gerechten, ' + tot + ') staat klaar: ' + tijd + ', Orthenstraat 53. Betalen doe je bij het afhalen, met pin of contant.'; },
  ...
  var mand = {};
  var bewaard = leesBlok('rancho-mand') || {};
  ...
  function bewaar() { bewaarBlok('rancho-mand', mand); }
  ...
  /* elke dag open: ma-do 16:30-22:00, vr-zo 16:00-22:30; kwartierstappen */
  var wijze = 'afhalen';
  function open(dag) { return true; }
  function weekendDag(dag) { return dag === 0 || dag === 5 || dag === 6; }
  function tijden() {
    var nu = new Date();
    var lijst = [];
    var dag = new Date(nu);
    for (var d = 0; d < 8 && lijst.length === 0; d++) {
      if (open(dag.getDay())) {
        var wknd = weekendDag(dag.getDay());
        var van = new Date(dag); van.setHours(16, wknd ? 0 : 30, 0, 0);
        var tot = new Date(dag); tot.setHours(wknd ? 22 : 21, wknd ? 0 : 30, 0, 0);
        var vroegst = new Date(Math.max(van.getTime(), nu.getTime() + 25 * 60000));
  ...
  function profiel() { return leesBlok('rancho-profiel'); }
  function laatste() { return leesBlok('rancho-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('rancho-profiel'); localStorage.removeItem('rancho-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('rancho-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
  ...
  stapAfronden.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      bewaarBlok('rancho-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('rancho-profiel', { naam: naam, telefoon: telefoon, adres: adres, postcode: postcode });
      } else {
