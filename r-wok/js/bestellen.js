/* R-wok: het online bestelsysteem, powered by Mettafel.
   Alleen afhalen. Zoeken en categoriechips, account-op-toestel
   (localStorage, geen wachtwoord) met bestel-opnieuw, en ?add=<id>
   vanuit de keuzematrix op de homepage.
...
(function () {
  ...
  var T = {
    leeg: 'Nog leeg. Kies iets lekkers van de kaart.',
    ...
    verstuurFout: function (m) { return 'Versturen lukte niet (' + m + '). Probeer het opnieuw, of bel ons op 073 689 6399.'; },
    versturen: 'Versturen...', plaats: 'Plaats bestelling',
    zsm: 'Zo snel mogelijk (± 10 minuten)',
    vandaag: 'Vandaag', tijdAfhaal: 'Afhaaltijd',
    bevestig: function (naam, n, tot, tijd) { return 'Dank je ' + naam + '! De receptkaarten liggen klaar. Je bestelling (' + n + (n === 1 ? ' item, ' : ' items, ') + tot + ') staat voor je klaar: ' + tijd + ', Maaspoortweg 285. Betalen doe je bij het afhalen, met pin of contant.'; },
    ...
    opnieuw: function (n, tot) { return 'Bestel opnieuw: ' + n + (n === 1 ? ' item · ' : ' items · ') + tot; },
  ...
  /* is er via ?add iets toegevoegd, open dan de categorie van dat gerecht */
  if (add && ITEMS[add]) {
    var regel = document.querySelector('.bestel-regel[data-item="' + add + '"]');
    if (regel) {
      var cat = regel.closest('.menu-categorie');
      if (cat) {
        cats.forEach(function (d) { d.open = d === cat; });
        setTimeout(function () { regel.scrollIntoView({ block: 'center' }); }, 60);
      }
    }
  }
  ...
  function bewaar() { bewaarBlok('rwok-mand', mand); }
  ...
  /* ---------- tijden binnen de echte openingstijden ----------
     ma en wo t/m zo 12:00-20:00; laatste moment 19:45, kwartierstappen.
     De wokrobot maakt het snel: 10 minuten voorbereidingstijd is genoeg. */
  function open(dag) { return dag !== 2; }
  function tijden() {
    var nu = new Date();
    var lijst = [];
    var dag = new Date(nu);
    for (var d = 0; d < 8 && lijst.length === 0; d++) {
      if (open(dag.getDay())) {
        ...
        var vroegst = new Date(Math.max(van.getTime(), nu.getTime() + 10 * 60000));
  ...
  function profiel() { return leesBlok('rwok-profiel'); }
  function laatste() { return leesBlok('rwok-laatste'); }
  ...
  if (accountPlek) {
    accountPlek.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-account-wis]')) {
        try { localStorage.removeItem('rwok-profiel'); localStorage.removeItem('rwok-laatste'); } catch (err) {}
        toonAccount();
      }
    ...
    accountPlek.addEventListener('submit', function (e) {
      ...
      bewaarBlok('rwok-profiel', { naam: f.naam.value.trim(), telefoon: f.telefoon.value.trim() });
      toonAccount();
    });
  }
  toonAccount();
  ...
  stapAfronden.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      bewaarBlok('rwok-laatste', { regels: mand, wanneer: new Date().toISOString() });
      if (profiel()) {
        bewaarBlok('rwok-profiel', { naam: naam, telefoon: telefoon });
      } else {
